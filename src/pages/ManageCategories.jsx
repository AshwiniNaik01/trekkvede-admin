...
import { FaLayerGroup, FaPlus, FaCheckCircle, FaTimesCircle, FaEdit, FaEye, FaTrashAlt, FaInfoCircle, FaImage } from "react-icons/fa";
...
export default function ManageCategories() {
    ...
    const handleDelete = async (row) => {
        if (window.confirm(`Are you sure you want to permanently delete category "${row.title}"?`)) {
            try {
                setLoading(true);
            ...
            render: (row) => (
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl border-2 border-emerald-100 overflow-hidden flex-shrink-0 bg-gray-50 flex items-center justify-center">
                        {row.catImage ? (
                            <img src={row.catImage} alt={row.title} className="w-full h-full object-cover" />
                        ) : (
        ...
        },
        {
            ...
            render: (row) => (
                <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-black text-amber-600 uppercase tracking-widest">{row.tag || "No Tag"}</span>
                    <span className="text-[10px] font-bold text-gray-400">{row.difficulty || "Standard"}</span>
                </div>
            ),
        },
        {
            ...
            <div className="max-w-7xl mx-auto space-y-8">
                <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-gray-500 font-medium mt-1">Organize your treks into meaningful collections</p>
                    </div>
                    <Link
                ...
                {error && (
                    <div className="bg-red-50 border border-red-100 text-red-600 px-6 py-4 rounded-2xl font-bold flex items-center gap-3 animate-in fade-in slide-in-from-top-4">
                        <FaTimesCircle />
                        {error}
                        <button onClick={fetchCategories} className="ml-auto underline">Try Again</button>
                    </div>
                )}
                        ...
                        <div className="flex flex-col md:flex-row gap-6">
                            <div className="w-full md:w-40 h-40 rounded-xl overflow-hidden border border-gray-100 flex-shrink-0">
                                {selectedCategory.catImage ? (
                                    <img src={selectedCategory.catImage} alt={selectedCategory.title} className="w-full h-full object-cover" />
                                ) : (
                            ...
                            <div className="flex-1 space-y-3">
                                ...
                                <p className="text-sm text-gray-500 font-medium">
                                    ID: <span className="text-gray-900 font-bold">{selectedCategory.categoryId}</span>
                                </p>
                                <div className="p-3 bg-emerald-50 rounded-lg border border-emerald-100 inline-block">
                                    <span className="text-xs font-black text-emerald-700 uppercase">{selectedCategory.trekCount} Total Treks Linked</span>
                                </div>
                            </div>
                        </div>
